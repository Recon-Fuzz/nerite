# Function: revokeFlowOperatorWithFullControl(contract ISuperfluidToken,address,bytes)
...
```
┌─ [0] ⚙️ FUNCTION: ConstantFlowAgreementV1.revokeFlowOperatorWithFullControl(contract ISuperfluidToken,address,bytes) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: ConstantFlowAgreementV1.updateFlowOperatorPermissions(contract ISuperfluidToken,address,uint8,int96,bytes) (NodeID: 1)
      💬 Args: [token, flowOperator, 0, 0, ctx]
      👁️  Def: public
