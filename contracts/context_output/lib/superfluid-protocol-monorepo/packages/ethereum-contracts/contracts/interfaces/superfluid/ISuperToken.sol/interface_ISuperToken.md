...
- **Name**: ISuperToken
- **Type**: Interface
- **Path**: lib/superfluid-protocol-monorepo/packages/ethereum-contracts/contracts/interfaces/superfluid/ISuperToken.sol
- **Documentation**:  @title Super token (Superfluid Token + ERC20 + ERC777) interface
   @author Superfluid
...
- **Signature**: `name()`
- **Visibility**: external
- **Source Range**: 3555:88:142

**Signature:**
```solidity
///  @dev Returns the name of the token.
function name() override(IERC777, IERC20Metadata) external view returns (string memory);;
```
...
- **Signature**: `symbol()`
- **Visibility**: external
- **Source Range**: 3756:90:142

**Signature:**
```solidity
///  @dev Returns the symbol of the token, usually a shorter version of the
///  name.
function symbol() override(IERC777, IERC20Metadata) external view returns (string memory);;
```
...
function decimals() override(IERC20Metadata) external view returns (uint8);;
```
...
- **Visibility**: external
- **Source Range**: 4842:81:142

**Signature:**
```solidity
///  @dev See {IERC20-totalSupply}.
function totalSupply() override(IERC777, IERC20) external view returns (uint256);;
```
...
- **Visibility**: external
- **Source Range**: 5017:101:142

**Signature:**
```solidity
///  @dev Returns the amount of tokens owned by an account (`owner`).
function balanceOf(address account) override(IERC777, IERC20) external view returns (uint256 balance);;
```
...
function transfer(address recipient, uint256 amount) override(IERC20) external returns (bool);;
```
...
function allowance(address owner, address spender) override(IERC20) external view returns (uint256);;
```
...
function approve(address spender, uint256 amount) override(IERC20) external returns (bool);;
```
...
///  @return Returns Success a boolean value indicating whether the operation succeeded.
///  @custom:emits a {Transfer} event.
function transferFrom(address sender, address recipient, uint256 amount) override(IERC20) external returns (bool);;
```
...
function granularity() override(IERC777) external view returns (uint256);;
```
...
///  interface.
function send(address recipient, uint256 amount, bytes calldata userData) override(IERC777) external;;
```
...
///  @custom:requirements
///  - the caller must have at least `amount` tokens.
function burn(uint256 amount, bytes calldata userData) override(IERC777) external;;
```
...
function isOperatorFor(address operator, address tokenHolder) override(IERC777) external view returns (bool);;
```
...
function authorizeOperator(address operator) override(IERC777) external;;
```
...
///  @custom:requirements
///  - `operator` cannot be calling address.
function revokeOperator(address operator) override(IERC777) external;;
```
...
function defaultOperators() override(IERC777) external view returns (address[] memory);;
```
...
///  - if `recipient` is a contract, it must implement the {IERC777Recipient}
///  interface.
function operatorSend(address sender, address recipient, uint256 amount, bytes calldata userData, bytes calldata operatorData) override(IERC777) external;;
```
...
function operatorBurn(address account, uint256 amount, bytes calldata userData, bytes calldata operatorData) override(IERC777) external;;
```
