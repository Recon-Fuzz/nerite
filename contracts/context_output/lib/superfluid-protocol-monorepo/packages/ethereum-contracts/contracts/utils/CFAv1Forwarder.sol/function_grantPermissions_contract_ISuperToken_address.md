# Function: grantPermissions(contract ISuperToken,address)
...
function grantPermissions(ISuperToken token, address flowOperator) external returns (bool) {
    return _updateFlowOperatorPermissions(token, flowOperator, FlowOperatorDefinitions.AUTHORIZE_FULL_CONTROL, type(int96).max);
}
```
...
```
┌─ [0] ⚙️ FUNCTION: CFAv1Forwarder.grantPermissions(contract ISuperToken,address) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  └─ [1] ⚙️ FUNCTION: CFAv1Forwarder._updateFlowOperatorPermissions(contract ISuperToken,address,uint8,int96) (NodeID: 1)
      💬 Args: [token, flowOperator, FlowOperatorDefinitions.AUTHORIZE_FULL_CONTROL, type(int96).max]
      👁️  Def: internal
