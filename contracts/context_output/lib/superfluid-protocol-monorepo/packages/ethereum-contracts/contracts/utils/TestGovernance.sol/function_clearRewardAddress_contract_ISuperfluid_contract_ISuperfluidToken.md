# Function: clearRewardAddress(contract ISuperfluid,contract ISuperfluidToken)
...
function clearRewardAddress(ISuperfluid host, ISuperfluidToken superToken) external {
    _clearConfig(host, superToken, SuperfluidGovernanceConfigs.SUPERFLUID_REWARD_ADDRESS_CONFIG_KEY);
...
```
┌─ [0] ⚙️ FUNCTION: SuperfluidGovernanceBase.clearRewardAddress(contract ISuperfluid,contract ISuperfluidToken) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
