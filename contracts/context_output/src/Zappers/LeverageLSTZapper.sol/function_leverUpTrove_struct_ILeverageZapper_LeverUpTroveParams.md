...
function leverUpTrove(LeverUpTroveParams calldata _params) external {
    ...
    flashLoanProvider.makeFlashLoan(collToken, _params.flashLoanAmount, IFlashLoanProvider.Operation.LeverUpTrove, abi.encode(_params));
...
```
┌─ [0] ⚙️ FUNCTION: LeverageLSTZapper.leverUpTrove(struct ILeverageZapper.LeverUpTroveParams) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
