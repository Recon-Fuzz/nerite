...
function leverDownTrove(LeverDownTroveParams calldata _params) external {
    ...
    flashLoanProvider.makeFlashLoan(collToken, _params.flashLoanAmount, IFlashLoanProvider.Operation.LeverDownTrove, abi.encode(_params));
...
```
┌─ [0] ⚙️ FUNCTION: LeverageLSTZapper.leverDownTrove(struct ILeverageZapper.LeverDownTroveParams) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
  ...
  │   💬 Args: [collToken, boldToken, initialBalances, receiver]
  │   👁️  Def: internal
