...
function openLeveragedTroveWithRawETH(OpenLeveragedTroveParams calldata _params) external payable {
    ...
    flashLoanProvider.makeFlashLoan(WETH, _params.flashLoanAmount, IFlashLoanProvider.Operation.OpenTrove, abi.encode(_params));
...
```
┌─ [0] ⚙️ FUNCTION: LeverageWETHZapper.openLeveragedTroveWithRawETH(struct ILeverageZapper.OpenLeveragedTroveParams) (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: external
